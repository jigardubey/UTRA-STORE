...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Authentication request could not be completed.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      if (email.trim()) {
        ...
        setError('Google popup preview iframe me constrained hai. Apni Google Email ID upar Email box me type karein aur Continue with Google pe click karein!');
      }
    }
  };
          ...
          <>
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full py-2.5 px-4 border border-gray-200 rounded-xl font-bold text-xs text-gray-800 hover:bg-gray-50 flex items-center justify-center gap-2.5 transition-colors shadow-2xs bg-white"
              >
